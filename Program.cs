...
namespace XslMail
{
	...
	class Program
	{
		...
		)
		{
            ...
            {
                SaveTempFile(step++, tempFilePathFormat, html);
            }
			...
			// STEP 4 (OPTIONAL): Perform substitutions.
			if (!String.IsNullOrEmpty(_options.Substitutions))
			{
				try
				{
					html = Substitute(_options.Substitutions, html);
				}
				catch (Exception ex)
				{
					throw new Exception(
						String.Format("Cannot perform substitutions '{0}'.", 
							_options.Substitutions),
						ex);
				}
		...
		/// Process step.
		/// </param>
        /// <param name="tempFilePathFormat">
		/// Temporary file path format.
		/// </param>
        /// <param name="html">
		/// HTML text.
		/// </param>
		...
		(
			int step, 
			string tempFilePathFormat, 
			string html
		)
        {
            string tempFilePath = String.Format(tempFilePathFormat, step);
		...
		/// String containing substitutions in format: value1=valueX|value2=valueY|value3=valueZ.
		/// </param>
        /// <param name="html">
		/// HTML text.
		/// </param>
        /// <returns>
		/// HTML text after string substitutions were completed.
		/// </returns>
        ...
        {
			Dictionary<string,string> keyValues = substitutions.Split(new[] {'|'}, StringSplitOptions.RemoveEmptyEntries)
               .Select(part => part.Split('='))
               .ToDictionary(split => split[0], split => split[1]);

			foreach(string key in keyValues.Keys)
			{
				html = html.Replace(key, keyValues[key]);
			}
